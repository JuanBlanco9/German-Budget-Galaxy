...
const RULES = [
  ...
  ["Children's Social Care", /\b(foster|fostering|adoption|adopt|looked[\s-]after|\blac\b|children\s*in\s*care|leaving\s*care|care\s*leaver|cwd|children[''s]*\s*residential|childrens\s*(placement|home|social)|cypl|cyp\s|young\s*people|youth\s*offending|youth\s*justice|cic\s|children\s*services|childrens?\s*social\s*care|csc\b|children[''s]*\s*(centre|team|placement)|troubled\s*families|sghc?\s*\(children|early\s*help|\bcwd\b|head\s*of\s*children|children\s*in\s*need|children\s*activities|subsistence\s*allowance\s*for\s*children|director\s*safeguarding|head\s*of\s*court\s*and\s*specialist|boarding\s*school\s*fees|contextual\s*safeguarding|early\s*intervention\s*team)/i],
  // --- Adult Social Care ---
  ["Adult Social Care", /\b(adult\s*(social\s*care|services|placements?|safeguarding)|asc\b|older\s*people|\bop\b[\s-](residential|nursing|home\s*care|care|comm)|learning\s*disab|physical\s*disab|mental\s*health\s*(adult|placement)|day\s*care\s*adult|domicil\w+\s*care|home\s*care|residential\s*(care|placement|home)|nursing\s*(care|home|placement)|direct\s*payments?\s*(asc|adult)|\bpdsi\b|\bpd\b[\s-]physical|\bdirect\s*payments\s*direct\s*cash|\blts\b|long[\s-]term\s*support|supported\s*(living|accommodation)|personal\s*budget|\bcarers?\b\s*(assess|grant)|dementia|advocacy\s*adult|reablement|head\s*of\s*learning\s*disab|head\s*of\s*hospital\s*and\s*localit|head\s*of\s*direct\s*care|\bhsc\b|health\s*and\s*social\s*care|director\s*of\s*adult|head\s*of\s*mental\s*health|occupational\s*therapy)/i],
  // --- Education (schools, teaching) ---
  ["Education", /\b(schools?\b[^.]*\b(block|secondary|primary|special|sen|sixth|fund|grant)|sen\b\s|special\s*educational|\bsend\b|dsg\b|high\s*needs|early\s*years|nursery|pupil|teacher|teaching|educational?\s*(psychology|welfare|service)|school\s*improvement|schools\s*fin|education\s*(dept|service)|\bed\s*(psychology|services)|children's?\s*centre|pvi\s*provider|catering\s*school|school\s*meals?|school\s*transport|children\s*with\s*disab\s*school|bsf\s*wave|basic\s*need|dedicated\s*schools?|head\s*of\s*(education|adult\s*education|\bsend\b)|head\s*of\s*schools|director\s*of\s*education)/i],
  // --- Transport ---
  ...
  // --- Housing ---
  ["Housing", /\b(housing\s*(revenue|rent|benefit|needs|repair|service|assist|strategy|general|hra|management|special\s*projects)|hra\b[\s-]?(repair|rent|tenant|property)|\bhra\s*operating|homeless|council\s*housing|housing\s*options|temporary\s*accomm|rent\s*(arrears|collect|account)|landlord|tenant|rsl\b|right\s*to\s*buy|leasehold|housing\s*stock|housing\s*repair|homeless|temp\s*accomm|decent\s*homes|disabled\s*facilities|dfg\b|housing\s*maint|director\s*housing|head\s*of\s*housing|capital\s*programme[^|]*housing|asset\s*management[^|]*housing|programme\s*and\s*asset\s*management.*housing)/i],
  // --- Public Health ---
